...
| 名前 | cron (UTC) | 意味 |
|---|---|---|
| 朝イチ判定ボード | `15 21 * * 0-5` | **月〜土** の 6:15 JST |
| 答え合わせ | `0 9 * * 1-5` | 平日 18:00 JST |

**UTC 21:15 は翌日の JST 6:15。** したがって UTC の曜日 `0-5`（日〜金）＝ JST の **月〜土**。

**土曜の朝は残す。** 金曜夜の米国市場と、第1金曜なら雇用統計の結果が入るため、週末版として価値がある（9/5がその例）。
**日曜の朝は落とす**（9/6に `* * *` から変更）。土日は東京も米国も休場で、土曜版から新しい材料が1つも増えないため。

プロンプトは「このスキル（jp-stock-research）に従って本日分を実行し、ボードを更新して PushNotification を送ること」で足りる。**長いプロンプトを書き直す必要はない。手順はこのスキルにある。**

## ⛔ body.html → cat → Artifact の3工程をやめる（9/17・3回目の失敗を受けて改訂）

**9/11・9/15・9/16と3回、夕方の答え合わせが未公開のまま朝を迎えた。**

**9/16に「組み立てたら先に公開する」と順番のルールを書いた。その翌日にまた同じところで止まった。順番の問題ではない。**

**止まる場所はいつも同じ ——「body.html を書いた直後の `cat head body tail > board.html`」の後である。** 工程が3つに分かれていて、2つ目と3つ目の間で切れている。

**したがって工程そのものを減らす。**

| | これまで | これから |
|---|---|---|
| 1 | `body.html` を Write | **`board.html` を Edit**（`</style>` と `<script>` の間の本文を丸ごと差し替える） |
| 2 | `cat head body tail > board.html` | — |
| 3 | Artifact で公開 | **Artifact で公開** |

**`board.html` は常に公開できる完全な状態で置いておく。** 差し替えは Edit 1回で済むので、途中で止まっても前日の完全なボードが残る。

**head.html / tail.html は復元用の雛形として残すが、毎回の組み立てには使わない。**

（未公開のまま朝になった場合は、内容を翌朝のボードに統合し、「出せなかった」ことも本文に書く。隠さない。これは変えない。）

## ⛔ ボードは「組み立てたら、まず公開する」（9/15追加・2回目の失敗）

**9/11と9/15の2回、夕方の答え合わせがボードを組み立てたところで止まり、公開もpushもされないまま朝を迎えた。**

**2回起きたので偶然ではない。** 組み立て（head+body+tail の結合）と公開（Artifact）の間で途切れている。

**したがって順番を固定する:**

1. **body.html を書く**
2. **結合して Artifact で公開する** ← **ここまでを最優先で終わらせる**
3. PushNotification
4. そのあとで 台帳・scoring.md・commit・push

**台帳や採点ルールの更新を先にやらない。** 公開が終わっていない状態で他の作業に入ると、そこで止まったときにユーザーの手元に何も残らない。

**朝のボードも同じ順番にする。**

（未公開のまま朝になった場合は、内容を翌朝のボードに統合し、「出せなかった」ことも本文に書く。隠さない。）
